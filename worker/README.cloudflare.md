...
>
> 另外：`static/_worker.js` 是 Pages Advanced Mode 的入口文件。Workers 部署时会被 `static/.assetsignore` 排除，避免被当成静态资源上传导致部署失败。
...
```bash
cd worker
npx wrangler pages deploy static --project-name <你的Pages项目名> --commit-dirty
```
