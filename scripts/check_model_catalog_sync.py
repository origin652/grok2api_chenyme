...
Checks:
1) app/services/grok/services/model.py and worker/src/grok/models.ts expose the exact same model ids.
2) worker/src/**/*.ts must not contain removed legacy model identifiers.
"""
...
PY_MODEL_FILE = ROOT / "app" / "services" / "grok" / "services" / "model.py"
TS_MODEL_FILE = ROOT / "worker" / "src" / "grok" / "models.ts"
TS_SRC_DIR = ROOT / "worker" / "src"
...
def main() -> int:
    ...
    if py_ids != ts_ids:
        ...
        if only_workers:
            lines.append("Only in worker/src/grok/models.ts:")
            lines.extend(f"  - {mid}" for mid in only_workers)
        errors.append("\n".join(lines))

    removed_hits = _scan_removed_identifiers()
    if removed_hits:
        lines = ["Removed model identifiers found under worker/src/**/*.ts:"]
